...
class QualityValidator:
    ...
    CRITERIA = {
        ...
        'background_balance': {
            ...
            'criterion': 2,
            'threshold': (0.8, 1.25),  # I∥/I⊥ ratio 範圍（嚴格）
    ...
    def validate_scene(self, report: dict, check_exr: bool = True) -> Dict[str, any]:
        ...
        if 'intensity_balance' in report:
            ...
            print(f"    [C2] ratio={ratio:.4f}, threshold={threshold}, pass={results['background_balance']}")
        else:
            results['background_balance'] = False
            print(f"    [C2] 缺少 intensity_balance 資料, pass=False")

        # Criterion 3: Glass Polarization Ratio (玻璃比值，雙向對比)
        if 'polarization' in report and 'glass_region' in report['polarization']:
            ...
            results['glass_ratio'] = False
            print(f"    [C3] 缺少 polarization/glass_region 資料, pass=False")
        ...
        if 'glass_depth_validity' in report:
            ...
            validity_rate = report['glass_depth_validity'].get('validity_rate', 0)
            print(f"    [C5] validity_rate={validity_rate:.4f}, pass={results['glass_depth_validity']}")
        else:
            results['glass_depth_validity'] = False
            print(f"    [C5] 缺少 glass_depth_validity 資料, pass=False")
    ...
    def generate_markdown_report(self, validation_results: Dict) -> str:
        ...
        lines.append("## 場景詳細結果")
        lines.append("")
        lines.append("| 場景 | C1 V-Disp | C2 背景 | C3 玻璃比值 | C4 對齊 | C5 深度 | 評分 | 結果 |")
        lines.append("|------|-----------|---------|---------|---------|---------|------|------|")
        ...
        for item in validation_results['results']:
            ...
            # C3: Glass Ratio (玻璃比值)
            c3_str = f"{'✓' if results.get('glass_ratio', False) else '✗'}"
        ...
        if failed_items:
            ...
            for item in failed_items:
                ...
                # Criterion 3: Glass Polarization Ratio (玻璃比值)
                if not results.get('glass_ratio', False):
                    failed_criteria.append('C3')
                    polarization = report.get('polarization', {})
                    glass_region = polarization.get('glass_region', {})
                    glass_ratio = glass_region.get('intensity_ratio_mean', 1.0)
                    glass_ratio_max = glass_region.get('intensity_ratio_max', 1.0)
                    pixel_count = glass_region.get('pixel_count', 0)
                    threshold_low, threshold_high = self.CRITERIA['glass_ratio']['threshold']
                    lines.append("#### ✗ Criterion 3: Glass Polarization Ratio (玻璃比值)")
                    lines.append("")
                    lines.append("**目的**: 確認玻璃區域 I∥/I⊥ 比值有足夠對比（正向或反向），表示偏振對比度足夠訓練使用。")
                    lines.append("")
                    lines.append("**測量結果**:")
                    lines.append(f"- 玻璃比值 (Mean): **{glass_ratio:.2f}x** (需 < {threshold_low}x 或 > {threshold_high}x)")
                    lines.append(f"- 玻璃比值 (Max): {glass_ratio_max:.2f}x")
                    lines.append(f"- 玻璃像素數: {pixel_count}")
                    lines.append("")
                    lines.append("**計算方法**: 使用視差將右圖 warp 到左視角，計算同一 3D 點的 I∥/I⊥ 比值。")
                    lines.append("")
                    lines.append("**可能原因**:")
                    lines.append("- 玻璃入射角接近正向 (Brewster 角效應不明顯)")
                    lines.append("- 偏振光源強度不足")
                    lines.append("- 環境光（非偏振）過強，稀釋偏振對比")
                    lines.append("- 玻璃材質設定問題")
                    lines.append("")
        ...
        lines.append("| 3 | Glass Polarization Ratio | 玻璃比值 I∥/I⊥ | < 0.96x 或 > 1.04x |")
        ...
        lines.append("")
        lines.append("#### Criterion 3: Glass Polarization Ratio (玻璃比值)")
        lines.append("**目的**: 確認玻璃區域 I∥/I⊥ 比值足夠高，表示偏振對比度足夠訓練使用")
        lines.append("")
        lines.append("**實施方法**:")
        lines.append("1. 使用視差將右圖 (I⊥) warp 到左視角")
        lines.append("2. 在左視角的玻璃遮罩區域計算 I∥ / I⊥_warped")
        lines.append("3. 計算玻璃區域的平均比值")
        lines.append("4. 判定: mean(glass_ratio) < 0.96x 或 > 1.04x（雙向對比）")
        lines.append("")
        lines.append("```python")
        lines.append("# 玻璃比值計算 (warp 對齊後)")
        lines.append("I_cross_warped = warp_right_to_left(I_cross, disparity)")
        lines.append("glass_ratio = I_parallel[glass_mask_left] / I_cross_warped[glass_mask_left]")
        lines.append("mean_ratio = glass_ratio.mean()  # 應 < 0.96 或 > 1.04")
        lines.append("```")
        lines.append("")
        lines.append("#### Criterion 4: Ground Truth Alignment (深度對齊)")
...
def main():
    ...
    parser.add_argument('--passed', type=str, default='passed_scenes.txt',
                        help='輸出通過場景列表 (預設: passed_scenes.txt)')
