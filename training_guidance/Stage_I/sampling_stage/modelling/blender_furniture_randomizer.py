...
5. 使用旋轉邊界盒 (Rotated AABB) 計算，杜絕穿模。
"""
...
CONFIG = {
    ...
    # [設定] 檔案輸出路徑 (如果不存在會自動建立)
    # 建議使用絕對路徑，例如: "C:/Dataset/Output" 或 "./generated_scenes"
    ...
    'scene': {
        ...
        'y_min': 528,      # 玻璃放置最小深度
        'y_max': 695,      # 玻璃放置最大深度
        'y_bg_base': 730,  # 傢俱放置基準線
        'y_bg_jitter': (-10, 10), # 基準線隨機偏移範圍
    },
    
    # 玻璃物件設定
    'glass_objects': {
        'count_range': (1, 3), # 每個場景生成 1~3 個玻璃
        'types': [
    ...
    # 傢俱物件設定
    'furniture': {
        'count_range': (2, 4), # 每個場景生成 2~4 個傢俱
        'types': [
    ...
    # 隨機化參數
    'randomization': {
        'initial_max_scale': 1.05,     # 初始最大嘗試縮放倍率
        'min_scale_limit': 0.6,        # 縮小極限 (低於此值則放棄放置)
        'glass_rotation_y': (-30, 30), # 玻璃 Y 軸旋轉範圍
        'glass_rotation_x': (-5, 5),   # 玻璃 X 軸微調
        'furniture_rotation_z': (-15, 15), # 傢俱 Z 軸旋轉範圍
    },
}

# ============================================================
# 2. 核心數學 (Math & Collision)
# ============================================================

def get_rotated_size(w, d, angle_rad):
    """
    計算矩形旋轉後的 Axis-Aligned Bounding Box (AABB) 尺寸
    """
