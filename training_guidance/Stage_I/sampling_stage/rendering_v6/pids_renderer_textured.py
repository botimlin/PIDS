"""
PIDS Stage 1 Renderer v6.0.0 (Polarized RGB)
=============================================

Fork from v5.1.8，從灰階偏振輸出升級為 RGB 偏振輸出。

v6.0.0 更新:
-----------
1. Film pixel_format 從 'luminance' 改為 'rgb'
   - Mitsuba spectral_polarized 內部處理 spectral → RGB 轉換
   - Stokes integrator 輸出 15 channels: RGB(3) + S0(3) + S1(3) + S2(3) + S3(3)
2. I_parallel 和 I_cross 現為 3 通道 RGB (H,W,3) 而非灰階 (H,W)
   - 解決 Pol Volume Crosstalk 的灰階瓶頸問題
   - 為 backbone 提供 RGB 特徵，提升 stereo matching 區分力
3. EXR 輸出改為 3 通道 RGB 格式
4. PNG 預覽改為彩色圖像
5. DoLP 計算先轉 luminance 再算（DoLP 本質是標量）
6. QA pipeline 內部轉 luminance 處理，保持向後相容

v5.x 歷史:
---------
- v5.1.8: glass_mask_strict, rerender-strict-mask
- v5.1.7: _is_glass_name() 精確匹配, rerender-mask
- v5.0.0: 天花板光降低, DoLP Stokes 計算, Glass Mask 聯集
- v4.0.0: MTLParser 紋理支持
    ...
    def _create_sensor(self,
        ...
        return {
            ...
            'film': {
                ...
                'pixel_format': 'rgb',  # v6: RGB 格式（spectral → RGB 由 Mitsuba 內部轉換）
...
class StokesProcessor:
    ...
    def rgb_to_luminance(rgb: np.ndarray) -> np.ndarray:
        """RGB (H,W,3) → luminance (H,W)，使用 ITU-R BT.709 係數"""
        if rgb.ndim == 2:
            return rgb  # 已經是灰階
    ...
    def extract_stokes(image: np.ndarray) -> Tuple[np.ndarray, np.ndarray, np.ndarray]:
        ...
        v6: rgb film + stokes integrator 輸出 15 channels:
            Channels 0-2:   RGB (standard render output)
            Channels 3-5:   S0 (intensity, RGB)
            Channels 6-8:   S1 (linear pol H/V, RGB)
            Channels 9-11:  S2 (linear pol diagonal, RGB)
            Channels 12-14: S3 (circular pol, RGB)

        Returns:
            (S0, S1, S2) - 各為 (H, W, 3) RGB array
        """
        if image.ndim != 3:
        ...
        if n_channels == 15:
            # v6 標準格式: rgb film + stokes integrator
            # RGB(3) + S0(3) + S1(3) + S2(3) + S3(3) = 15
            print(f"  [Stokes] v6 RGB Stokes 格式 (15ch)")
            S0 = image[:, :, 3:6]    # S0 RGB
            S1 = image[:, :, 6:9]    # S1 RGB
            S2 = image[:, :, 9:12]   # S2 RGB

        elif n_channels == 12:
            # 備用: S0(3) + S1(3) + S2(3) + S3(3) = 12 (無前綴 RGB)
            print(f"  [Stokes] RGB Stokes 格式 (12ch, 無前綴)")
            S0 = image[:, :, 0:3]
            S1 = image[:, :, 3:6]
            S2 = image[:, :, 6:9]

        else:
            # Fallback: v5 灰階邏輯（安全網）
            print(f"  [Stokes] 警告: 非預期通道數 {n_channels}，使用 v5 灰階 fallback")
            if n_channels >= 9:
                n_spectral = n_channels // 3
                S0_gray = np.mean(image[:, :, 0:n_spectral], axis=2)
                S1_gray = np.mean(image[:, :, n_spectral:2*n_spectral], axis=2)
                S2_end = min(3*n_spectral, n_channels)
                S2_gray = np.mean(image[:, :, 2*n_spectral:S2_end], axis=2)
            elif n_channels == 4:
                S0_gray = image[:, :, 0]
                S1_gray = image[:, :, 1]
                S2_gray = image[:, :, 2]
                ...
                S0_gray = 0.2126 * image[:,:,0] + 0.7152 * image[:,:,1] + 0.0722 * image[:,:,2]
                S1_gray = np.zeros_like(S0_gray)
                S2_gray = np.zeros_like(S0_gray)
            else:
                raise ValueError(f"不支援的通道數: {n_channels}")
            # 灰階 → 假 RGB (複製到 3 通道以統一下游介面)
            S0 = np.stack([S0_gray]*3, axis=2)
            S1 = np.stack([S1_gray]*3, axis=2)
            S2 = np.stack([S2_gray]*3, axis=2)

        # 輸出統計 (用 luminance 做摘要)
        S0_lum = StokesProcessor.rgb_to_luminance(S0)
        S1_lum = StokesProcessor.rgb_to_luminance(S1)
        S2_lum = StokesProcessor.rgb_to_luminance(S2)
        print(f"  [Stokes] S0 luminance 範圍: [{S0_lum.min():.4f}, {S0_lum.max():.4f}]")
        print(f"  [Stokes] S1 luminance 範圍: [{S1_lum.min():.4f}, {S1_lum.max():.4f}]")
        print(f"  [Stokes] S2 luminance 範圍: [{S2_lum.min():.4f}, {S2_lum.max():.4f}]")
    ...
    def compute_polarization_images(S0: np.ndarray,
        ...
        v6: S0, S1, S2 為 (H,W,3) RGB，輸出 I∥, I⊥ 也為 (H,W,3) RGB
        """
        ...
        # 輸出統計 (使用 luminance 做摘要)
        I_par_lum = StokesProcessor.rgb_to_luminance(I_parallel)
        I_cross_lum = StokesProcessor.rgb_to_luminance(I_cross)
        diff_lum = np.abs(I_par_lum - I_cross_lum)
        print(f"  [偏振] I∥ luminance 範圍: [{I_par_lum.min():.4f}, {I_par_lum.max():.4f}]")
        print(f"  [偏振] I⊥ luminance 範圍: [{I_cross_lum.min():.4f}, {I_cross_lum.max():.4f}]")
        print(f"  [偏振] |I∥-I⊥| luminance 平均: {diff_lum.mean():.4f}, 最大: {diff_lum.max():.4f}")
    ...
    def compute_dolp(S0: np.ndarray, S1: np.ndarray, S2: np.ndarray) -> np.ndarray:
        ...
        v6: RGB Stokes (H,W,3) → luminance → DoLP (H,W)
        DoLP 本質是標量，先轉 luminance 再計算
        """
        S0_lum = StokesProcessor.rgb_to_luminance(S0)
        S1_lum = StokesProcessor.rgb_to_luminance(S1)
        S2_lum = StokesProcessor.rgb_to_luminance(S2)
        dolp = np.sqrt(S1_lum**2 + S2_lum**2) / (S0_lum + 1e-10)
    ...
    def compute_dolp_from_intensities(I_parallel: np.ndarray, I_cross: np.ndarray) -> np.ndarray:
        """從 I∥ 和 I⊥ 計算偏振度 DoLP (標量 H,W)

        v6: RGB (H,W,3) → luminance → DoLP (H,W)
        """
        I_par_lum = StokesProcessor.rgb_to_luminance(I_parallel)
        I_cross_lum = StokesProcessor.rgb_to_luminance(I_cross)
        I_sum = I_par_lum + I_cross_lum
        dolp = np.zeros_like(I_par_lum)
        valid_mask = I_sum > 1e-6
        dolp[valid_mask] = np.abs(I_par_lum[valid_mask] - I_cross_lum[valid_mask]) / I_sum[valid_mask]
        dolp = np.clip(dolp, 0, 1)
        return dolp.astype(np.float32)
...
def generate_scene_report(scene_name: str,
    ...
    # v6: QA 報告中的標量指標使用 luminance (與 v5 相容)
    I_par_lum = StokesProcessor.rgb_to_luminance(I_parallel)
    I_cross_lum = StokesProcessor.rgb_to_luminance(I_cross)
    ...
    # ============================================================
    # 1. 數值範圍 (v6: 使用 luminance)
    # ============================================================
    report['value_range'] = {
        'I_parallel': {
            'min': float(np.min(I_par_lum)),
            'max': float(np.max(I_par_lum)),
            'mean': float(np.mean(I_par_lum)),
        },
        'I_cross': {
            'min': float(np.min(I_cross_lum)),
            'max': float(np.max(I_cross_lum)),
            'mean': float(np.mean(I_cross_lum)),
        },
    }
    ...
    # v6: Warp luminance I_cross 到左視角（QA 指標使用標量）
    I_cross_warped, warp_valid = warp_right_to_left(I_cross_lum, disparity)

    # 偏振差異（用於 SNR 計算）- 使用 warp 後的對齊圖像
    diff = np.abs(I_par_lum - I_cross_warped)

    # 強度比值（使用 warp 後的 I_cross，比較同一 3D 點）
    ratio_mask = (I_cross_warped > 0.01) & warp_valid
    intensity_ratio = np.zeros_like(I_par_lum)
    if np.any(ratio_mask):
        intensity_ratio[ratio_mask] = I_par_lum[ratio_mask] / I_cross_warped[ratio_mask]
    ...
    noise_parallel = estimate_noise(I_par_lum)
    noise_cross = estimate_noise(I_cross_lum)
    ...
    if np.any(bg_balance_mask):
        bg_intensity_ratio = I_par_lum[bg_balance_mask] / I_cross_warped[bg_balance_mask]
        bg_ratio_mean = float(np.mean(bg_intensity_ratio))
        bg_ratio_std = float(np.std(bg_intensity_ratio))
        bg_mean_parallel = float(np.mean(I_par_lum[bg_balance_mask]))
...
class PIDSRenderer:
    ...
    def render_scene(self, obj_path: str, output_dir: str, scene_name: str = None):
        ...
        I_0deg = 0.5 * (S0_left + S1_left)  # (H,W,3) RGB
        I_90deg = 0.5 * (S0_left - S1_left)  # (H,W,3) RGB
        I_0deg = np.maximum(I_0deg, 0)
        I_90deg = np.maximum(I_90deg, 0)

        # v6: 偏振比值是標量指標，轉 luminance 計算
        I_0deg_lum = StokesProcessor.rgb_to_luminance(I_0deg)    # (H,W)
        I_90deg_lum = StokesProcessor.rgb_to_luminance(I_90deg)  # (H,W)

        valid_pol_mask = (I_0deg_lum > 0.01) & (I_90deg_lum > 0.01)
        true_pol_ratio = np.ones_like(I_0deg_lum)
        if np.any(valid_pol_mask):
            true_pol_ratio[valid_pol_mask] = I_90deg_lum[valid_pol_mask] / I_0deg_lum[valid_pol_mask]
    ...
    def _save_outputs(self,
        ...
        params = {
            ...
            'stats': {
                # v6: 使用 luminance 做範圍統計 (與 v5 報告格式相容)
                'I_parallel_range': [float(StokesProcessor.rgb_to_luminance(I_parallel).min()),
                                     float(StokesProcessor.rgb_to_luminance(I_parallel).max())],
                'I_cross_range': [float(StokesProcessor.rgb_to_luminance(I_cross).min()),
                                  float(StokesProcessor.rgb_to_luminance(I_cross).max())],
    ...
    def _save_png(self, image: np.ndarray, path: str, gamma: float = 2.2):
        """保存 PNG (自動範圍 + gamma correction, v6: 支援 RGB)"""
        if image.ndim == 3 and image.shape[2] == 3:
            # RGB PNG
            ...
            cv2.imwrite(path, cv2.cvtColor(normalized, cv2.COLOR_RGB2BGR))
        else:
            if image.ndim == 3:
                image = image[:, :, 0]
            vmin = np.percentile(image, 1)
    ...
    def _save_png_fixed(self, image: np.ndarray, path: str, vmin: float, vmax: float, gamma: float = 2.2):
        """保存 PNG (固定範圍 + gamma correction, v6: 支援 RGB)"""
        if image.ndim == 3 and image.shape[2] == 3:
            # RGB PNG (scalar vmin/vmax broadcast to all channels)
...
範例:
  ...
  python pids_renderer_textured.py --rerender-strict-mask ./output --obj-dir ./scenes --output ./output --scene-list train_scenes.txt
    ...
    if args.num_gpus > 1:
        ...
        skipped = 0
        for i, scene_path in enumerate(scenes):
            # 斷點續渲：若 report.json 已存在，跳過此場景
            scene_stem = scene_path.stem  # e.g. "scene_0001"
            report_file = Path(args.output) / f"{scene_stem}_report.json"
            if report_file.exists():
                skipped += 1
                if skipped <= 5 or skipped % 100 == 0:
                    print(f"  [跳過] {scene_stem} (已渲染, 累計跳過 {skipped})")
                continue

            print(f"\n[進度] {i+1}/{len(scenes)} (跳過 {skipped})")
            Config.randomize_for_augmentation(seed=deterministic_hash(scene_path.name))
            try:
