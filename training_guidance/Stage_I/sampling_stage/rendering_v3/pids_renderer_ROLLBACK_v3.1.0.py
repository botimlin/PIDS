...
--------
    基線: 65mm
    FOV: 65°
    左相機: X = -32.5mm, 0° 偏振片 (I∥)
    右相機: X = +32.5mm, 90° 偏振片 (I⊥)
...
class Config:
    ...
    # 相機位置 (在 chamber 內)
    CAMERA_Y = 360.0                # 相機 Y 位置 (略大於前牆，在 chamber 內)
    CAMERA_Z = 150.0                # 相機高度 (chamber 高度的一半)
    ...
    def left_camera_position(cls) -> Tuple[float, float, float]:
        """左相機位置"""
        return (-cls.BASELINE / 2, cls.CAMERA_Y, cls.CAMERA_Z)
...
class PIDSRenderer:
    ...
    def render_scene(self, obj_path: str, output_dir: str, scene_name: str = None):
        ...
        輸出:
            - {scene}_left_parallel.exr  (I∥ 灰階)
            - {scene}_right_cross.exr    (I⊥ 灰階)
        ...
        print(f"[配置] 左相機: {left_pos}")
        print(f"[配置] 右相機: {right_pos}")
        ...
        # 渲染左相機 (I∥) - 使用 0° 偏振片
        print(f"\n[1/4] 渲染左相機 (I∥, θ=0°)...")
        left_image = self._render_camera(builder, left_pos, target, polarizer_angle=0.0)
        ...
        # 渲染右相機 (I⊥) - 使用 90° 偏振片
        print(f"\n[2/4] 渲染右相機 (I⊥, θ=90°)...")
        right_image = self._render_camera(builder, right_pos, target, polarizer_angle=90.0)
        ...
        # 渲染深度圖
        print(f"\n[3/4] 渲染深度圖...")
        depth = self._render_depth(builder, left_pos, target)
    ...
    def _save_outputs(self,
        ...
        # 生成品質報告
        print(f"\n[Report] 生成品質報告...")
        report = generate_scene_report(scene_name, I_parallel, I_cross, depth)
...
def main():
    parser = argparse.ArgumentParser(
        description='PIDS Stage 1 Renderer v3.0',
...
範例:
  ...
  # 批次渲染
  python pids_renderer.py --input_dir ./scenes --output ./output --max_scenes 100
