...
------------------
    基線: 65mm
    FOV: 65°
    左相機: X = -32.5mm, 0° 偏振片 (I∥), 目標 = (-32.5, 611.5, 150)
    右相機: X = +32.5mm, 90° 偏振片 (I⊥), 目標 = (+32.5, 611.5, 150)
...
class PIDSRenderer:
    ...
    def render_scene(self, obj_path: str, output_dir: str, scene_name: str = None):
        ...
        # 渲染深度圖（使用左相機視角）
        print(f"\n[3/4] 渲染深度圖...")
        depth = self._render_depth(builder, left_pos, left_target)
...
def main():
    parser = argparse.ArgumentParser(
        description='PIDS Stage 1 Renderer v3.2.0 (平行光軸配置)',
...
範例:
  ...
  python pids_renderer.py --scene scene.obj --output ./output --spp 8192
    ...
    print(f"[PIDS Renderer v3.2.0 - 平行光軸]")
