"""
PIDS Stage 1 Renderer v3.6.1 (Low Memory - 3060 Ti 適用)
========================================================

從頭設計的偏振立體渲染器，用於生成 PIDS 訓練數據。
Fork from v3.6.0，針對低顯存 GPU (8GB) 優化。

v3.6.1 更新 (Low Memory):
------------------------
- 降低 SPP_PER_BATCH 從 1024 → 256 (64 批次而非 16 批次)
- 每批次顯存需求降低約 4 倍
- 新增 --batch_spp 參數可自訂批次大小
- 新增 --ultra_low 模式 (batch=128, 適用 6GB GPU)
- 渲染時間增加約 50% (批次切換開銷)，但可在 8GB GPU 上穩定運行

v3.6.0 更新:
-----------
- 移除 right_parallel.exr 渲染（用於 C1 檢查，但 --skip-c1 時不需要）
- 渲染步驟從 6 步減少到 5 步
- 節省約 16% 渲染時間
...
class Config:
    ...
    SPP_PER_BATCH = 256   # 分批渲染，避免 GPU OOM (64 批次，適用 8GB GPU)
    MAX_DEPTH = 12        # 光線反彈次數
...
範例:
  ...
  python pids_renderer_random.py --input_dir ./scenes --output ./output --num_gpus 4
    ...
    parser.add_argument('--batch_spp', type=int, default=Config.SPP_PER_BATCH,
                        help=f'每批次 SPP (預設: {Config.SPP_PER_BATCH}，越小顯存需求越低)')
    parser.add_argument('--ultra_low', action='store_true',
                        help='超低顯存模式 (batch_spp=128，適用 6GB GPU)')
    ...
    Config.SPP = args.spp
    if args.ultra_low:
        Config.SPP_PER_BATCH = 128
        print("[低顯存模式] --ultra_low: batch_spp=128 (128 批次)")
    else:
        Config.SPP_PER_BATCH = args.batch_spp
    ...
    n_batches = Config.SPP // Config.SPP_PER_BATCH
    print(f"[PIDS Renderer v3.6.1 - Low Memory Mode]")
    print(f"  隨機化: LED強度, 環境光, 相機X位置")
    print(f"  GPU 數量: {args.num_gpus}")
    print(f"  SPP: {Config.SPP}, 批次: {n_batches} x {Config.SPP_PER_BATCH}")
    if args.skip > 0:
